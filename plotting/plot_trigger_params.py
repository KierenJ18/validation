...
for subfolder in subfolders:
    ...
    try:
        ...
        x_labels = ['eventnumber', 'triggernumber', 'triggertype', '# hits', 'ntubeshitdigi',
                    'ndigihitstrigger', 'triggertime', 'digipeperdigi','digitimeperdigi', 'totaldigipe',
                    'totaldiginoisefrac', 'digipe', 'Time (ns)', 'Digit Noise Time (ns)', 'digitime_photon',
                    'digitime_mix', 'digiplustriggertime', 'diginoisefrac']
        yscales = ['linear', 'linear', 'linear', 'linear', 'linear',
                   'linear', 'linear', 'linear', 'linear', 'linear', 
                   'linear', 'linear', 'linear', 'log', 'linear',
                   'linear', 'linear', 'linear']
        
        # Plot various parameters
        for i in range(len(per_trigger.fields)):
            list_keys = list(per_trigger.fields)
            ...
            per_trigger_filtered = ak.flatten(per_trigger[list_keys[i]].compute(), axis=None)
            
            plot_and_save_variable(
                per_trigger_filtered,
                var_name=list_keys[i],
