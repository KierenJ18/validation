		 ...
		 int max_ntriggers = -1)
{
  ...
  if (mkdir("results", 0755) != 0 && errno != EEXIST) {
    std::cerr << "Error creating results directory! Errno: " << errno << std::endl;
    return -1; // Exit if directory creation fails
} else {
    std::cout << "Results directory is ready." << std::endl;
}

TString filenameTString(filename);

TString outputFilePath = TString::Format("results/%s", create_filename(TString::Format("analysed_%s", inbranch), filenameTString, ".root").Data());
