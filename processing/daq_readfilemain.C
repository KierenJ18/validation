  ...
  const char *results_dir = "results")
{
  ...
  if (mkdir(results_dir, 0755) != 0 && errno != EEXIST) {
    std::cerr << "Error creating results directory: " << results_dir << "! Errno: " << errno << std::endl;
    return -1; // Exit if directory creation fails
} else {
    std::cout << "Results directory is ready: " << results_dir << std::endl;
}

TString filenameTString(filename);
TString outputFilePath = TString::Format("%s/%s", results_dir, create_filename(TString::Format("analysed_%s", inbranch), filenameTString, ".root").Data());
...
fout->cd();
  ...
  float_t fWCCylRadius, fWCCylLength;
  tout_file->Branch("npmt20", &npmt20);
  tout_file->Branch("npmtm", &npmtm);
  tout_file->Branch("npmtod", &npmtod);
  tout_file->Branch("WCCylRadius", &fWCCylRadius);
  tout_file->Branch("WCCylLength", &fWCCylLength);
  //per event variables
  ...
  fWCCylRadius = geo->GetWCCylRadius();
  fWCCylLength = geo->GetWCCylLength();
  tout_file->Fill();
  ...
  for (int ev=0; ev<nevent; ev++)
  {
    ...
    for (int itrack = 0; itrack < ntracks; itrack++)
    {
      ...
      tvtrack_parentid.push_back(wcsimroottrack->GetParentId());
      ...
      tvtrack_startpos.push_back(TVector3(wcsimroottrack->GetStart(0),
          ...
          tvdirection.push_back(TVector3(wcsimroottrack->GetDir(0),
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " <filename> <verbose> <inbranch> <results_dir>" << std::endl;
        return -1;
    }

    daq_readfile(argv[1], atoi(argv[2]), argv[3], 999999999999, -1, argv[4]);
